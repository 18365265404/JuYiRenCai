    ...
    <div class="person1 content">
        ...
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style="background: #66B1ff; padding-left: 2%;line-height: 50px;">
                <span class="el-icon-info"></span><span>网站管理</span><span>/企业表单</span>
         </div>
        </div>
        ...
        <div class="table" style=" font-size:14px">
		    ...
		    <tr class="thead-tr">
              ...
              <th style="text-align: center;">经营面积</th>
              <th style="text-align: center; ">企业类型</th>
              <th style="text-align: center;">企业规模</th>
              <th style="text-align: center; ">经营方式</th>
              <th style="text-align: center; ">需要人才</th>
           ...
           <tbody>
            <tr class="tbody-tr" v-for="(item ,index) of list" :key="item.id">
                ...
                <td style="text-align: center;">{{item.industryInvolved}}</td>
                <td style="text-align: center;">{{item.enterpriseName}}</td>
                <td style="text-align: center;">{{item.personPhone}}</td>
                <td style="text-align: center;">{{item.businessAddress}}</td>
                <td style="text-align: center;">{{item.businessArea}}</td>
                <td style="text-align: center;">{{item.enterpriseType}}</td>
                <td style="text-align: center;">{{item.enterpriseScale}}</td>
                <td style="text-align: center;">{{item.businessPractice}}</td>
                <td style="text-align: center;">{{item.needTalent}}</td>
                <td style="text-align: center;">{{item.createTime}}</td>
                <td style="text-align: center;  width:100px; position:relative">
                    ...
                    <div  v-show="!item.uploadPhotos" style="width:95px; height:35px; font-size:10px;  margin:5px;overflow:hidden;  position: absolute;left:0px;top:10px">
                        <el-upload
                            ...
                            class="upload-demo"
                            :action="upExcelurlImg+'?enterpriseUUID='+item.id"
        ...
        <div v-show="list.length!=0" class="chooseAll" style="width:90%; box-sizing: border-box; margin:0 auto; margin-bottom:2%; border:1px solid black; border-top:none; position:relative">
...
export default {
     ...
     methods:{
            ...
            downExcel(){
                ...
                window.location.href ="http://pe5iwubyw.bkt.clouddn.com/example1.xls"
            },
            // 导出Excel表
            changeTode1(index){
                 var getCookies=getCookie("token")
                 var urls=`${url.root()}${url.bcinExpertExcel()}?token=${getCookies}&idStr=${this.list[index].id}`
                 ...
                 for(let i=0;i<this.checkedCities.length;i++){
                    ...
                    this.$confirm('是否要批量导出, 是否继续?', '提示', {
                        ...
                        type: 'warning'
                            ...
                            var getCookies=getCookie("token")
                            var urls=`${url.root()}${url.bcinExpertExcel()}?token=${getCookies}&idStr=${chooseArrId}`
   ...
   //控制checkbox复选框
    ...
    //    console.log(this.checkedCities,"打印的checkedCities值")
     ...
     created:function(){
            //获取上传Excel表格接口
            this.upExcelurl=url.root()+url.bcinImportExcel()
